      ...
      <div class="max-w-2xl mx-auto px-4 text-center">
        ...
        <h4 class="font-bold text-xl leading-snug">
          <span class="whitespace-nowrap">
            ${item.ja} <span class="font-eng text-blue-400">${item.price}</span>
          </span>
        </h4>
        <p class="text-sm text-gray-400 font-eng mt-1">${item.en}</p>
        <p class="text-sm text-gray-300 mt-3 tracking-wide">${item.time}</p>

        <ul class="text-base text-gray-200 space-y-2 mt-6 text-left leading-relaxed list-disc pl-5 w-full">
          ${items}
        </ul>
