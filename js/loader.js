export function initLoader() {
    ...
    // すでにセッションにフラグがある（リロード等）場合は、即座に削除
  ...
  `;
    ...
    function openDoors() {
        ...
        setTimeout(() => {
            if (isMobile) a.style.transform = "translateY(-110%)";
            else a.style.transform = "translateX(-110%)";
        }, 120);
        setTimeout(() => {
            if (isMobile) b.style.transform = "translateY(110%)";
            else b.style.transform = "translateX(110%)";
        }, 240);
        setTimeout(() => {
            if (line) line.remove();
...
function injectStyle() {
  ...
  #center-line { position: absolute; width: 1px; height: 100%; left: 50%; top: 0; background: rgba(255,255,255,0.2); z-index: 20; will-change: transform, opacity; backface-visibility: hidden; }
  ...
  .door::after { content: ""; position: absolute; inset: 0; box-shadow: inset 0 0 40px rgba(0,0,0,0.4); }
  ...
  @media (max-width: 768px) {
    ...
    .door-a, .door-b { width: 100%; height: 50%; }
    .door-a { top: 0; }
    .door-b { top: 50%; }
  }
  `;
